...
|-----------|------|------------|--------|
| SDIO | PC8, PC9, PC10, PC11, PC12, PD2 | SD-карта (4-bit mode) | Настроено |
...
|--------|-----|-----|------------|
| K0 | PE4 | Active-low, pull-up | Маркер в логе (EXTI falling, 300ms any-edge debounce, D2 мигает ~75мс) |
| K1 | PE3 | Active-low, pull-up | Shutdown (EXTI) |
| K_UP | PA0 | Active-high, pull-down | Не используется |
| RST | NRST | — | Аппаратный сброс MCU |

## Текущий прототип CAN входа

Модифицированная плата MCP2515 (контроллер MCP2515 не используется, только трансивер TJA1050):

```
  STM32 F4VE                   Модифицированная MCP2515 плата
  ──────────                   ──────────────────────────────
  PB9 (CAN1_TX) ──────────► SO (был SPI MISO, теперь TJA TXD)
  PB8 (CAN1_RX) ◄────────── SI (был SPI MOSI, теперь TJA RXD)
  GND ──────────────────────── GND
  5V ───────────────────────── VCC
```

Сигналы CAN TX/RX подключены к пинам SO/SI платы MCP2515, которые напрямую идут к TJA1050 трансиверу (MCP2515 контроллер обойдён).

## Hat (расширительная плата, план)
...
- **Кнопки маркера и shutdown** с RC-debounce (см. ниже)

CAN bus: 500 kbit/s, терминация 120 Ом на каждом конце шины.

### RC-debounce для кнопок (на hat)

Встроенные K0/K1 на отладочной плате использовались как временные. На hat разводятся дублирующие кнопки с hardware debounce — параллельно тем же пинам (PE4 для маркера, PE3 для shutdown), чтобы код не менялся.

Схема на каждую кнопку:
```
  PE4/PE3 ──┬──[ R (1–10 кОм) ]──┬── BTN ── GND
            │                    │
           === C (100 нФ)        │
            │                    │
           GND                  (опц.)
...
- **C = 100 нФ** от пина на GND (керамика, у самого connector'а hat).
- **R = 1–10 кОм** последовательный — опционально, усиливает фильтрацию; без него работает за счёт внутреннего pull-up STM32 (~40 кОм) × 100 нФ ≈ 4 мс постоянная времени на размыкании, <20 мкс на замыкании.
- Триггер Шмитта в GPIO STM32 дочистит сигнал — дребезги короче 1–2 мс не проходят в EXTI.
- Отладочные кнопки на основной плате после сборки production просто не монтировать (или оставить для диагностики).

На ревизии PCB hat достаточно заложить футпринты под R+C возле connector'а для всех кнопочных пинов — на макете их можно не ставить, софт-debounce (300 мс lockout в `HAL_GPIO_EXTI_Callback`) справится временно.
