...
Это описано в `docs/SD_ERRORS.md` как основная причина отказов записи.
...
1. Добавить счётчики ошибок через `HAL_SD_ErrorCallback` override
2. Вывести в CDC status для мониторинга
3. Записывать FAULT файл на SD при фатальной ошибке
4. Собрать данные с реального запуска — определить точно какая команда фейлит и как часто
