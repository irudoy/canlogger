...
DSTATUS SD_status(BYTE lun)
{
   ...
   * PROGRAMMING. osDelay(1) yields the current FreeRTOS task for 1 tick,
   * letting higher-priority tasks (CAN drain, demo gen) continue while
   * the SD card finishes its GC stall. This is the key decoupling point. */
...
/* Instrumented replacement for SD_write. Uses the RTOS osMessageQueue
 * for DMA completion (SDQueueID, populated by BSP_SD_WriteCpltCallback)
 * instead of the bare-metal WriteStatus polling. This lets the task yield
 * while waiting for DMA and card GC to finish. */
#undef SD_write
DRESULT SD_write(BYTE lun, const BYTE *buff, DWORD sector, UINT count)
{
  DRESULT res = RES_ERROR;
  ...
  if (!((uint32_t)buff & 0x3))
  {
    ...
    if (BSP_SD_WriteBlocks_DMA((uint32_t*)buff,
                               ...
                               count) == MSD_OK)
    {
      /* Wait for DMA Tx complete via RTOS message queue (yields task). */
      status = osMessageQueueGet(SDQueueID, (void *)&event, NULL, SD_TIMEOUT);
      if ((status != osOK) || (event != WRITE_CPLT_MSG))
      {
       ...
       * osDelay(1) yields to higher-priority tasks during GC stalls. */
      timeout = HAL_GetTick();
